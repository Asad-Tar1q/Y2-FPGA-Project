...
# ── Networking and scene packing ───────────────────────────────────────────────
# Change this if your PYNQ board has a different IP address.
HOST       = "192.168.2.99"
FRAME_PORT = 5000
CTRL_PORT  = 5001
SEND_HZ    = 30
...
def _connect_ctrl():
    ...
    try:
        ...
        with _ctrl_lock:
            _ctrl_sock = None
        print(f"[client] control unavailable ({e.__class__.__name__}); will retry on send")
        return False


def _send_ctrl_payload(payload):
    global _ctrl_sock, _last_ctrl_try
    packet = struct.pack(">I", len(payload)) + payload
    with _ctrl_lock:
        s = _ctrl_sock
    if s is None:
        now = time.monotonic()
        if now - _last_ctrl_try < 1.0:
            return False
        _last_ctrl_try = now
        if not _connect_ctrl():
            return False
        with _ctrl_lock:
            s = _ctrl_sock
    try:
...
def _wall_orientation(w):
    return "h" if abs(w.x2 - w.x1) >= abs(w.y2 - w.y1) else "v"


def _wall_endpoints_i16(w):
    x1, y1, x2, y2 = _snap_wall(w.x1, w.y1, w.x2, w.y2)
    return _i16(x1), _i16(y1), _i16(x2), _i16(y2)


def _moving_velocity_q4_4(app, ant):
    if not ant.moving:
        return 0, 0
    ax, ay = ant.move_a
    bx, by = ant.move_b
    period = max(float(ant.move_period), 0.1)
    phase = (app._move_time % period) / period
    direction = 1.0 if phase < 0.5 else -1.0
    vx_px_s = (bx - ax) * (2.0 / period) * direction
    vy_px_s = (by - ay) * (2.0 / period) * direction
    vx_px_tick = vx_px_s / ASSUMED_HW_FPS_FOR_VEL
    vy_px_tick = vy_px_s / ASSUMED_HW_FPS_FOR_VEL
    return _i16(vx_px_tick * 16.0), _i16(vy_px_tick * 16.0)


def _source_record(app, ant, *, virtual=False, wall_id=0, phase_inv=False, amp_scale=1.0, x=None, y=None):
    x = ant.x if x is None else x
    y = ant.y if y is None else y
    vx, vy = (0, 0) if virtual else _moving_velocity_q4_4(app, ant)
    theta = float(ant.theta_0)
    dirx = _i16(math.cos(theta) * 127.0)
    diry = _i16(math.sin(theta) * 127.0)
    flags = 0x01
    if ant.moving and not virtual:
        flags |= 0x02
    if virtual:
        flags |= 0x04
    if phase_inv:
    ...
    return struct.pack(
        SRC_REC_FMT,
        _i16(x), _i16(y), vx, vy,
        _u8(max(0.0, min(1.0, ant.amplitude * amp_scale)) * 255.0),
        _u8(max(0.0, min(15.9375, ant.frequency)) * 16.0),
        0,
        _u8(max(0.0, min(1.0, ant.a)) * 255.0),
        dirx, diry,
        flags,
        int(wall_id) & 0xFF,
    )
...
def _build_scene_payload(app):
    walls_hw = []
    for w in app.walls[:MAX_WALLS]:
        x1, y1, x2, y2 = _wall_endpoints_i16(w)
        wtype = 1 if w.wall_type == "reflect" else 0
        gain = 160 if wtype == 1 else 0
        phase_inv = 1 if wtype == 1 else 0
        walls_hw.append(struct.pack(WALL_REC_FMT, x1, y1, x2, y2, 1, wtype, gain, phase_inv))

    src_recs = []
    real_ants = list(app.antennas)
    for ant in real_ants:
        if len(src_recs) >= MAX_HW_SOURCES:
            break
        src_recs.append(_source_record(app, ant))

    # First-order image-source reflections. Moving sources are intentionally not
    # reflected in this version; the hardware applies retarded-time correction to
    # direct moving sources only.
    for wall_idx, w in enumerate(app.walls[:MAX_WALLS]):
        if w.wall_type != "reflect":
            continue
        x1, y1, x2, y2 = _wall_endpoints_i16(w)
        ...
        for ant in real_ants:
            ...
            if orient == "v":
                ...
                vx = ant.x
                vy = 2.0 * y1 - ant.y
            src_recs.append(_source_record(
                app, ant,
                virtual=True,
                wall_id=wall_idx,
                phase_inv=True,
    ...
    flags = 0x01 if _paused else 0x00
    header = struct.pack(SCENE_HDR_FMT, SCENE_MAGIC, SCENE_VERSION, flags,
                         len(src_recs), len(walls_hw), 0)
    return header + b"".join(src_recs) + b"".join(walls_hw)
